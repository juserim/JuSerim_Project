	...
	<div class="container-fluid tm-mt-60">
		<div class="row tm-mb-50">
			<div class="my-col-lg-4 col-12 mb-5">
				<h2 class="tm-text-primary mb-5">답장</h2>
				<form id="contact-form" action="reply" method="post" class="tm-contact-form mx-auto">
					<input type="hidden" name="sender" value="${ loginuser.userNo } ">
					<div class="form-group">
					<h4>받는사람</h4>
						<input type="text" readonly 
							   value="${ param.receiverId }" 
							   class="form-control rounded-0">
						<input type="hidden" name="receiver" value="${ param.receiverNo }"> 	   	  
							  
					</div>
					<%-- <div class="form-group">
							<select class="form-control" name="receiver">
						     <c:forEach var= "user" items="${ requestScope.userList }">
						      	<option value="${ user.userNo }">${ user.userId }</option>
						      </c:forEach>
							</select>
					</div> --%>
					<div class="form-group">
					...
					<div class="form-group">
						<textarea rows="8" name="content" class="form-control rounded-0"
							placeholder="내용" required></textarea>
					</div>

					<div class="form-group tm-text-right">
						<button type="submit" class="btn btn-primary">답장보내기</button>
					</div>
				</form>
			</div>
		</div>

...
@WebServlet(urlPatterns = { "/dogbreedintro/dogbreedintrowrite.action" })
public class DogBreedIntroWriteServlet extends HttpServlet {
	
	// get 요청 처리 ( 여기서는 글쓰기 화면 요청 )
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/dogBreedIntro/dogBreedIntrowrite.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//System.out.println("dogbreedintrowrite.action 글쓰기 처리 요청 !!!!!");
		...
		if (!ServletFileUpload.isMultipartContent(req)) {
			resp.sendRedirect("dogbreedintrolist.action");
			return;
		}

		//업로드된 파일을 저장할 경로 문자열을 저장할 변수
			...
			for (FileItem item : items) {
				...
				if (item.isFormField()) { //form-data인 경우 (File이 아닌 일반 데이터인 경우)
					...
					if (fileName != null && fileName.length() > 0) { //내용이 있는 경우
						...
						DogBreedIntroAttach boardAttach = new DogBreedIntroAttach();						

...
@WebServlet(urlPatterns = { "/showmydog/showmydogwrite.action" })
public class ShowMyDogWriteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/showmydog/showmydogwrite.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (!ServletFileUpload.isMultipartContent(req)) {
			resp.sendRedirect("showmydoglist.action");
			return;
		}
			...
			for (FileItem item : items) {
				...
				if (item.isFormField()) { //form-data인 경우 (File이 아닌 일반 데이터인 경우)
					...
					if (fileName != null && fileName.length() > 0) { //내용이 있는 경우
						...
						ShowMyDogBoardAttach boardAttach = new ShowMyDogBoardAttach();						
